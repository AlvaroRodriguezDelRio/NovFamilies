...
def process_fam(line):
    ...
    fam, nseqs, nspcs, src, members = map(str.strip, line.split('\t'))

    family_doc = {'gf': fam, 'neighs': [], 'size':int(nseqs), 'ntaxa': int(nspcs)}
    ...
    for pos in cons_per_position:
        for db in cons_per_position[pos]:
            for annot,number in cons_per_position[pos][db].items():
                array_to_report.append ('\t'.join([fam,str(pos),db,annot,str(number),str(number_genes_per_pos[pos]),nseqs,','.join(neigh_number_per_pos[pos][annot]),','.join(neigh_number_per_pos_neg[pos][annot]),','.join(distances_dict[pos][annot]),str(strand_flag_per_pos[pos][annot])]))

    return array_to_report


for ln, line in enumerate(open("/data/jhc/cold/MAGs/novel_fams-v2/clustering/microbial_genomes-v1.clustering.folded.parsed.NoISO.3sp.tsv")):
